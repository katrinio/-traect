...
Миграции `0005`–`0008` сведены в одну миграцию `0008_minimum_acceptable_level` поверх `0004_historical_domain_names`. Её идентификатор сохранён для совместимости с уже обновлёнными базами. Базы на удалённых промежуточных ревизиях `0005`–`0007` нужно сначала обновить предыдущим релизом, а затем развернуть эту версию.
