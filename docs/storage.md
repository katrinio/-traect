...
Alembic migrations сведены в одну baseline-миграцию `0008_minimum_acceptable_level`. Её идентификатор сохранён для совместимости с уже обновлёнными базами. Базы на промежуточных ревизиях прежней цепочки нужно сначала обновить предыдущим релизом, а затем развернуть эту версию.
